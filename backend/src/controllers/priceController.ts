...
async function fetchBasePrice(): Promise<number> {
  const row = await Setting.findOne({ where: { key_name: 'base_price' } });
...
export class PriceController {
  ...
  async updateBasePrice(req: Request, res: Response) {
    try {
      ...
      await Setting.upsert({ key_name: 'base_price', value: price.toString() });
